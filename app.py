...
mongo = PyMongo(app, uri="mongodb://localhost:27017/store_inventory")
...
def db_data(my_type):
    ...
    db_data = mongo.db.produce.find({'type':my_type}, {'_id': False})
    print('this route was pinged')
    parsed = [x for x in db_data]
    print('parsed: ', parsed)
    return jsonify(parsed)

@app.route('/cost-gt/<cost>')
def db_cost(cost):

    db_data = mongo.db.produce.find({'cost': {'$gt': float(cost)}}, {'_id': False})
    parsed = [x for x in db_data]
    print('parsed: ', parsed)
    return jsonify(parsed)
